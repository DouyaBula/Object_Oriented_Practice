...
public class Term implements Element, Serializable {
    ...
    public boolean isOnlyTrigonometric() {
    ...
    public void addFactor(Factor factor) throws CloneNotSupportedException {
        addFactor('+', factor);
    }
    ...
    private void addPower(Factor factor) throws CloneNotSupportedException {
        String base;
        base = ((Power) factor).getBase();
        if (factors.get(base) == null) {
            factors.put(base, factor);
        } else {
            Power factorNew = ((Power) factors.get(base)).clone();
            factorNew.setExponent(factorNew.getExponent()
                    + ((Power) factor).getExponent());
            factors.replace(base, factorNew);
        }
        this.powerExpanded = ((Power) factors.get(base)).isExpanded();
    }
