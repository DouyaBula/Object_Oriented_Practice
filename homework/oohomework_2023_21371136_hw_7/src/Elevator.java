...
public class Elevator extends Thread {
    ...
    public Elevator(int id, int floorNum, Parameter parameter,
                    ArrayList<Entry> inRequests, HashMap<Integer, Info> transferInfo,
                    ArrayList<Semaphore> semaphoresM, ArrayList<Semaphore> semaphoresN) {
        this.elevatorID = id;
        ...
        this.semaphoresM = semaphoresM;
        this.semaphoresN = semaphoresN;
        this.onlyIn = false;
    }
    ...
    private void open() throws InterruptedException {
        semaphoresM.get(pos).acquire();
        if (onlyIn) {
            semaphoresN.get(pos).acquire();
        }
    ...
    private void close() throws InterruptedException {
        ...
        semaphoresM.get(pos).release();
        if (onlyIn) {
            semaphoresN.get(pos).release();
            onlyIn = false;
        }
        opened = false;
    }
    ...
    private void check() throws InterruptedException {
        synchronized (inRequests) {
            ...
            // 检测是否为只进人
            onlyIn = outRequests.get(pos).size() == 0;
            // 出
        ...
        while (!outRequests.get(pos).isEmpty()) {
            ...
            synchronized (transferInfo) {
                if (transferInfo.get(passengerID) != null) {
                    if (LOG) {
                        System.out.println("电梯" + elevatorID + "将换乘乘客" + passengerID + "踢出");
                    }
        ...
        synchronized (inRequests) {
            for (int i = 0; i < floorNum; i++) {
                ...
                for (Integer passengerID :
                    ...
                    synchronized (transferInfo) {
                        boolean transfer = transferInfo.get(passengerID) != null;
                        if (transfer) {
                            Info info = transferInfo.get(passengerID);
                            inRequests.get(pos).addPassenger(info.getDest(), passengerID);
                        } else {
